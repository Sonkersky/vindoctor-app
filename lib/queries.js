...
function applyActiveFilters(query, filters) {
  ...
  // nieskończoność, sprawiając wrażenie, że nic się nie zmienia.
  //
  // UWAGA: to był wcześniej .or('auction_date.is.null,auction_date.gte.…') —
  // znalezione po fakcie: taki OR w połączeniu z ORDER BY auction_date na
  // liczbie wierszy rzędu setek tysięcy (vehicle_type='Automobile',
  // większość active_lots) dawał 14+ sekund na zapytanie (na wąskim
  // podzbiorze, np. Motorcycle, było szybko — stąd tylko strona główna
  // odczuwalnie zwolniła, nie /motorcycles).
  //
  // Próba naprawy przez wyliczaną kolumnę (auction_date_sort, full table
  // rewrite) dwukrotnie wywaliła produkcyjną bazę (brak miejsca na dysku
  // na WAL przy przepisywaniu 700k+ wierszy) — porzucone. Zamiast tego:
  // zwykły warunek zakresowy BEZ OR-a (loty z auction_date=NULL po prostu
  // nie pojawiają się w "Actual", dopóki apicar.store nie przypisze im
  // daty — i tak i tak lądowały na końcu sortowania ASC, więc realnie nic
  // nie tracimy) — w pełni indeksowalne zwykłym B-tree indeksem
  // (vehicle_type, auction_date), bez żadnej kolumny ani przepisywania
  // tabeli.
  return q.gte('auction_date', new Date().toISOString());
}
...
export const getCarByVin = cache(async function getCarByVin(vin) {
  ...
  const [
    ...
    // "cars" ma "vin" jako unique w schemacie — .maybeSingle() tu bezpieczne.
    supabase.from('cars').select(CAR_COLUMNS).eq('vin', vin).maybeSingle(),
    // "active_lots" NIE ma takiego ograniczenia (klucz to lot_id, nie vin) —
    // to samo auto potrafi być wystawione jako dwa osobne loty naraz.
    // Znalezione na żywo (zgłoszone przez Google Search Console jako "Błąd
    // serwera 5xx" na konkretnych stronach lota): .maybeSingle() rzuca
    // PGRST116 ("Results contain 2 rows... requires 1 row") i wywala całą
    // stronę na 500, gdy trafi taki duplikat. order+limit(1) zamiast
    // maybeSingle() po prostu bierze najświeższy wpis zamiast się wykładać.
    supabase
      .from('active_lots')
      .select(ACTIVE_LOT_COLUMNS)
      .eq('vin', vin)
