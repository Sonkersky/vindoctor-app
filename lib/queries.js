...
async function pageHasResults(supabase, filters, targetPage) {
  const offset = (targetPage - 1) * PAGE_SIZE;
  const check = applyFilters(
...
async function findLastPageInWindow(supabase, filters, currentPage, hasNextPage) {
  ...
  if (await pageHasResults(supabase, filters, windowEnd)) return windowEnd;

  let hi = windowEnd;
  while (hi - lo > 1) {
    const mid = Math.floor((lo + hi) / 2);
    if (await pageHasResults(supabase, filters, mid)) {
      lo = mid;
    } else {
      hi = mid;
    }
...
export async function listCars(filters = {}, page = 1) {
  ...
  const lastPageInWindow = await findLastPageInWindow(supabase, filters, page, hasNextPage);
