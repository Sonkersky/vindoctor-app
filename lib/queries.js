...
function applyFilters(query, filters) {
  ...
  // widoczne w Archive z czerwoną plakietką "Not sold" (patrz
  // tileStatusPillClass w app/page.js), zamiast być całkowicie ukryte.
  return applyCommonFilters(query.in('sale_status', [SOLD_VALUE, NOT_SOLD_VALUE]), filters);
}
...
// który usuwa stąd loty w chwili sprzedaży).
...
// (range(offset,offset) + count:'exact', head:true) — to samo, co już było
// używane dla hasNextPage, tylko wydzielone do ponownego użycia poniżej.
// Parametryzowane po tabeli/funkcji filtrującej, żeby ta sama logika
// obsługiwała zarówno "cars" (Archive) jak i "active_lots" (Actual).
async function pageHasResults(supabase, table, applyFn, filters, targetPage) {
  const offset = (targetPage - 1) * PAGE_SIZE;
  const check = applyFn(
    supabase.from(table).select('lot_id', { head: true, count: 'exact' }),
    filters
  );
  const { count } = await check.range(offset, offset);
  return (count || 0) > 0;
}
...
export async function listActiveLots(filters = {}, page = 1) {
  ...
  let hasNextPage = false;
  if (cars.length === PAGE_SIZE) {
    const nextCheck = applyActiveFilters(
      supabase.from('active_lots').select('lot_id', { head: true, count: 'exact' }),
      filters
    );
    const { count } = await nextCheck.range(to + 1, to + 1);
    hasNextPage = (count || 0) > 0;
