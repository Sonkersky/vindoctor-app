...
export const getLotCounts = unstable_cache(
  async function getLotCounts() {
    ...
    const [copartRes, iaaiRes] = await Promise.all([
      supabase
        .from('cars')
    ...
    const copart = copartRes.count || 0;
    const iaai = iaaiRes.count || 0;

    // Total liczony jako suma Copart + IAAI (jedyne dwie wartości base_site)
    // zamiast osobnego trzeciego zapytania — gwarantuje spójność liczb i
    // omija zaobserwowany na Vercelu problem z tym konkretnym zapytaniem.
    return { total: copart + iaai, copart, iaai };
  },
  ['lot-counts'],
  { revalidate: 300 }
);
