...
// Surowe wartości "make" w active_lots są dużo brudniejsze niż w "cars"
// (setki wpisów typu "H", "N/a", "Unknown", ucięte "Acur"/"Hond"/"Toyo" —
// sprawdzone na żywo). Czyszczenie u źródła nie ma sensu — apicar.store i
// tak dosyła nowy brud przy każdej synchronizacji. Zamiast tego: sekcja
// "Popular makes" pokazuje tylko sprawdzone, poprawnie zapisane marki z tej
// listy (przecięcie z tym, co faktycznie jest w danych) — ten sam efekt co
// prośba klienta o listę jak na referencyjnym screenie, bez potrzeby
// utrzymywania rozrastającej się tabeli literówek na zawsze.
const POPULAR_MAKES = {
  Automobile: [
    ...
    'Mini', 'Mitsubishi', 'Nissan', 'Oldsmobile', 'Peugeot', 'Polestar', 'Pontiac', 'Porsche',
    'Ram', 'Range Rover', 'Renault', 'Rivian', 'Rolls-Royce', 'Saab', 'Saturn', 'Scion',
    'Smart', 'Subaru', 'Suzuki', 'Tesla', 'Toyota', 'Triumph', 'Volkswagen', 'Volvo',
  ],
  Motorcycle: [
    'BMW', 'Ducati', 'Harley-Davidson', 'Honda', 'Husqvarna', 'Hyosung', 'Indian', 'Kawasaki',
    'Ktm', 'Kymco', 'Moto Guzzi', 'Piaggio', 'Polaris', 'Royal Enfield Motors', 'Suzuki',
    'Triumph', 'Ural', 'Vespa', 'Victory', 'Yamaha', 'Zero Motorcycles Inc',
  ],
  ATV: [
    'Arctic Cat', 'BMW', 'Bobcat', 'Can-Am', 'Cfmoto', 'Chevrolet', 'Dodge', 'Hisun', 'Honda',
    'Jeep', 'John Deere', 'Kawasaki', 'Kubota', 'Kymco Usa Inc', 'Polaris', 'Segway', 'Subaru',
    'Suzuki', 'Tgb', 'Tracker', 'Yamaha',
  ],
};

// Marki do sekcji "Popular makes" na stronie głównej — grupowane po
// vehicle_type (Automobile/Motorcycle/ATV), z widoku active_lot_makes
// (patrz supabase/schema.sql), przefiltrowane do POPULAR_MAKES (patrz
// wyżej). Cache'owane jak wyżej — ta sama lista jest identyczna dla
// każdego odwiedzającego.
export const getActiveLotMakes = unstable_cache(
  async function getActiveLotMakes() {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.from('active_lot_makes').select('vehicle_type,make').limit(2000);
    if (error) throw error;

    const present = { Automobile: new Set(), Motorcycle: new Set(), ATV: new Set() };
    for (const row of data || []) {
      if (present[row.vehicle_type]) present[row.vehicle_type].add(row.make);
    }

    const grouped = {};
    for (const [vehicleType, popular] of Object.entries(POPULAR_MAKES)) {
      grouped[vehicleType] = popular.filter((make) => present[vehicleType].has(make));
    }
    return grouped;
  },
  ['active-lot-makes'],
